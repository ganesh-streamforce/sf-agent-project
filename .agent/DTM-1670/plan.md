...
- **Field Metadata Declaration:**
  - `fullName`: `Account.Company_Address__c`
  - `type`: `Text`
...
- **Custom Field:** `Account.Company_Address__c`
...
- **Negative Testing:** Attempt to create an Account without filling this field (should succeed as optional). Attempt to enter >255 characters (should be truncated or rejected).
...
- **Modify Page Layout:**
  ...
  - Place the new `Company_Address__c` field in an appropriate section (e.g., "Custom Fields").
- **Set Field Security:**
